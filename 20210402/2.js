 ...
 *      + ...
 *     原理：所有数据类型值在计算机底层存储的时候，都是基于二进制进行存储的「根据IEEE-754 使用64位2进制值存储」；所有对象数据类型值前三位都是000，null的值是000000...；typeof检测是按照二进制值来检测的「处理速度是比较快的」，它认为以000开头的都是“object”「其中包含了null，而函数是经过特殊处理的{能够调用call方法的是函数}」
 *     不仅如此，我们平时进行数学运算的时候，也是按照底层存储的二进制值来进行运算处理的！！
 *     问题：0.1 + 0.2 !== 0.3   => 0.1/0.2本身存储的二进制是经过省略的，最后相加的结果也会存在精度差
 *     
 *  2. instanceof 本意是用来检测当前实例对象是否率属于这个类，并不是数据类型检测，只不过我们依托于他的特点，可以检测数据类型「临时拉来充壮丁的，所以准确度不好」
 ...
 *       @2 不论是基于这个方法处理，还是没有这个方法，最后本质都是：检测当前实例obj的原型链上是否会出现Ctor这个构造函数，如果可以出现，则结果是true，找到Object.prototype都没有出现过Ctor，则结果是false
 */
...
let f = new Fn;
console.log(f instanceof Array); //true  但是f本质上不应该称为数组「不具备数组应有的结构」 
*/

/* console.log(1 instanceof Number); //false  基于instanceof进行处理，并不会把原始值进行装箱处理，也就是原始值类型本身是不具备原型链的，所以instanceof是不能处理原始值类型的
