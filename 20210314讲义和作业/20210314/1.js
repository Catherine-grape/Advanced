 ...
 *        @1 在私有上下文中，有一个后期存储当前上下文中声明的变量的地方“私有变量对象AO” 「AO是VO的分支，在函数中，变量对象是AO」
 *        @2 进栈执行
 *     2. 在代码执行之前，他还会处理很多的事情
 *        @1 初始作用域链[[scope-chain]]：<自己形成的上下文,函数的作用域{创建时候声明的}>
 *           作用：但凡代码执行的时候，遇到一个变量，首先看是否是自己上下文中的私有变量(AO)，如果是自己的，接下来对变量的操作，和外界没有任何的关系；如果不是自己私有的，则继续按照作用域链，查找是否为其上级上下文(也就是函数作用域)中的...如果查找过程中一直都没有，找到EC(G)就结束即可
 *        @2 初始化this指向
 *        @3 初始化arguments「实参集合」
